...
//Because internal's scope is anywhere inside of myFunction, anything inside of myFunction has access to it.
...
function lowerCaseNames(arr) {
  const lowerCaseNames = arr.map(item => item.animal_name.toLowerCase())
...
function lowPopulationAnimals(arr) {
  const lowPopulationAnimals = arr.filter(item => item.population < 5)
...
function USApop(arr) {
  const USApop = arr.reduce((total, item) => total + item.population, 0)
  return USApop
}
